        ...
        <div>
            ...
            <Modal isOpen={modal} toggle={toggle} >
                ...
                <ModalBody>
                    ...
                    <select defaultValue='' value={updatedTask.status} onChange={(e) => setUpdatedTask({...updatedTask, status: e.target.value})}>{statuses.map(status => <option key={status.id} value={status.status}>{status.status}</option>)}</select>
